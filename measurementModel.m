...
Z(3)    = (p.P/p.THETA)*acos((p.xm*(p.xs-xc) + p.ym*(p.ys-yc) + p.zm*(p.zs-zc) - xc*p.xs - yc*p.ys - zc*p.zs + xc^2 + yc^2 + zc^2)/(sqrt((p.xs-xc)^2+(p.ys-yc)^2+(p.zs-zc)^2)*sqrt((p.xm-xc)^2+(p.ym-yc)^2+(p.zm-zc)^2)));
